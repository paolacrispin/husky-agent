...
export function huskyBrandingLines(version = getHuskyAgentVersion()): string[] {
  return [
    `${ICE}  /\_/\\${RESET}   ${BLUE}Husky Agent${RESET} ${MUTED}v${version}${RESET}`,
    `${ICE} ( o.o )${RESET}   ${MUTED}${HUSKY_TAGLINE}${RESET}`,
    `${ICE}  > ^ <${RESET}   ${MUTED}Try: “show wallet status” or “show balances”${RESET}`,
  ];
}
